...
export default class Lexer {
  ...
  _unquote(str: string): string {
    ...
    const stringWithoutQuotes: string = str.substr(1, str.length - 2)
    var unescapedString: string = ""

    for (let i = 0; i < stringWithoutQuotes.length; ++i) {
      if (stringWithoutQuotes[i] == '\\' && i+1 < stringWithoutQuotes.length && (stringWithoutQuotes[i+1] == '\\' || stringWithoutQuotes[i+1] == quote)) {
        ++i
      }
